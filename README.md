...
This widget provides real-time local weather and a beautiful 4-day forecast popup, featuring dynamically calculated moon phases with authentic NASA imagery. It supports dual backends - either the US National Weather Service (NWS) or the globally available Open-Meteo API.
...
But in that chasing, I found out it was far easier to not chase the DOM. I settled on plain old HTML/CSS cropping - specifically, a simple inline SVG with a circular `<clipPath>`. That process taught me an important lesson that became the philosophy for this entire widget: sometimes the easiest solution is the best one.
...
1. **The Data Source:** NASA's Scientific Visualization Studio (SVS) publishes an incredibly detailed "Dial-A-Moon" dataset every year (currently using SVS ID 5187 for 2024). NASA renders a high-resolution frame of the moon for every single hour of the entire year.
...
- **Self-Hosted Dashboards:** Both widgets natively link to included, fully responsive standalone dashboard pages (`full-forecast.html` and `full-forecast-gov.html`) powered entirely by Vanilla JS and their respective APIs - no third-party website links required.
...
- **[Weather Underground](https://www.wunderground.com/):** For providing the hyper-local fallback weather links.
- **National Weather Service (NWS):** For providing reliable US weather data.
- **NASA:** For the stunning astronomical moon imagery.
...
## Disclaimer
I am not affiliated, associated, authorized, endorsed by, or in any way officially connected with the National Weather Service (NWS), Open-Meteo, Weather Underground, NASA, or any of their subsidiaries or affiliates. All data and imagery are pulled from their respective public APIs and services.
