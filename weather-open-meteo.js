...
class WeatherWidget {
  constructor(elementId, options = {}) {
    ...
    this.lat = options.lat || 51.5074; // default to London for global example
    this.lon = options.lon || -0.1278;
  ...
  async loadWeather() {
    try {
      ...
      this.element.onclick = () => {
        const locName = encodeURIComponent(this.locationName || 'Local Forecast');
        window.open(`full-forecast.html?lat=${this.lat}&lon=${this.lon}&name=${locName}`, '_blank', 'noopener,noreferrer');
      };
      ...
      const url = `https://api.open-meteo.com/v1/forecast?latitude=${this.lat}&longitude=${this.lon}&current=temperature_2m,weather_code,is_day&daily=weather_code,temperature_2m_max,temperature_2m_min&temperature_unit=fahrenheit&timezone=auto`;
      
      const response = await Promise.race([
        fetch(url),
        timeoutPromise
      ]);
      if (!response.ok) throw new Error('Failed to fetch Open-Meteo data');
      const data = await response.json();

      const tempF = Math.round(data.current.temperature_2m);
      const weatherCode = data.current.weather_code;
      
      const desc = this.getWMODescription(weatherCode);
      const iconData = this.getWeatherIcon(desc, null);
      
      this.locationName = this.options.locationName || this.options.name || 'Local Forecast';

      let forecastPeriods = [];
      if (data.daily) {
        for(let i=0; i<Math.min(4, data.daily.time.length); i++) {
          const shortDesc = this.getWMODescription(data.daily.weather_code[i]);
          const high = Math.round(data.daily.temperature_2m_max[i]);
          const low = Math.round(data.daily.temperature_2m_min[i]);
          forecastPeriods.push({
            name: this.getDayName(data.daily.time[i]),
            shortForecast: shortDesc,
            detailedForecast: `${shortDesc}. High: ${high}°F, Low: ${low}°F.`,
            temperature: high,
            isDaytime: i === 0 ? !this.isNighttime() : true,
            startTime: data.daily.time[i]
          });
        }
      }

      const forecastText = `Currently: ${desc}`;
      
      this.renderWidget(iconData, tempF, forecastText, forecastPeriods, forecastPeriods[0]);
  ...
  renderWidget(iconData, tempF, tooltipText, forecastPeriods, activePeriod) {
    ...
    if (forecastPeriods && forecastPeriods.length > 0 && activePeriod) {
      ...
      displayPeriods.forEach((p, index) => {
        ...
        const fIconData = this.getWeatherIcon(p.shortForecast, !p.isDaytime, pDate);
  ...
  getWeatherIcon(description, overrideNight = null, targetDate = new Date()) {
